...
export async function schedule_update_start(
  request: NextApiRequest,
  response: NextApiResponse,
  infos: ScheduleCtx
) {
  const { id, start_at } = JSON.parse(request.body)

  if (!start_at || !id)
    return response.status(400).json({ error: 'Date and id is required' })
...
const helper = (request: NextApiRequest, response: NextApiResponse) =>
  scheduleMiddleWare(request, response, schedule_update_start)
