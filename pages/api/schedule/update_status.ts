...
export async function schedule_update_status(
  request: NextApiRequest,
  response: NextApiResponse,
  infos: ScheduleCtx
) {
  const { id, status } = JSON.parse(request.body)

  if (!status || !id)
    return response.status(400).json({ error: 'Status and id is required' })
...
const helper = (request: NextApiRequest, response: NextApiResponse) =>
  scheduleMiddleWare(request, response, schedule_update_status)
